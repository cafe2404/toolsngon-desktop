...
const TabInfo = (): React.JSX.Element => {
  ...
  const loadTabInfo = async (): Promise<void> => {
    ...
    try {
      const [
        ...
        desktop.browser.tabs.getCookies(currentTab.id),
        desktop.browser.tabs.getInfo(currentTab.id),
        desktop.browser.tabs.getSessionStorage(currentTab.id),
        desktop.browser.tabs.getLocalStorage(currentTab.id),
        desktop.browser.tabs.getIndexedDB(currentTab.id),
        desktop.browser.tabs.getWebSQL(currentTab.id),
        desktop.browser.tabs.getCache(currentTab.id)
      ])
  ...
  const [isOpen, setIsOpen] = useState<CopyId[]>([
    'proxy',
    'device',
    'indexeddb',
    'websql',
  ...
  const InfoSection = ({
    ...
    <div
      className={`text-sm overflow-y-hidden text-slate-800 p-2 bg-slate-100 rounded-md relative ${isOpen.includes(copyId) ? 'max-h-full' : 'max-h-24'}`}
    >
          ...
          <div className="space-y-3">
            <div className="text-sm overflow-y-hidden text-slate-800 p-2 bg-slate-100 rounded-md relative">
              {viewInfo.accountName && (
                <p>
                  <span className="font-medium">Tên:</span> {viewInfo.accountName}
                </p>
              )}
              {viewInfo.accountId && (
                <p>
                  <span className="font-medium">ID:</span> {viewInfo.accountId}
                </p>
              )}
              <p>
                <span className="font-medium">URL:</span> {viewInfo.currentUrl}
              </p>
            </div>
            {viewInfo.proxy && (
              <InfoSection
