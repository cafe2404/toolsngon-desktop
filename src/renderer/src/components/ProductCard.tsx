...
const ProductCard = ({ item }: { item: UserProduct }): JSX.Element => {
  ...
  return (
    <>
      {item.account_group &&
        item.account_group?.accounts.length > 0 &&
        item.account_group?.accounts &&
        item.account_group.accounts.map((account, i) => (
          <div
            key={i}
            className={`relative w-full white border border-slate-200 rounded-lg platform-item ${!account.is_active ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            {!account.is_active && (
            ...
            <div className="flex group gap-4 p-2 relative">
              ...
              <div className="py-2 relative w-full">
                <div className="space-y-1.5 w-full">
                  <h3
                    ...
                    className="cursor-pointer hover:underline line-clamp-1 text-lg font-semibold leading-6 text-slate-800 "
                  >
                  ...
                  <div className="text-sm text-slate-600 font-medium flex items-center gap-2">
                    <Clock size={16}></Clock>
                    {t('product.expiresOn', {
                      date: new Date(item.end_date).toLocaleDateString(dateLocale)
                    })}
                  </div>
                  <div className="text-sm text-slate-600 font-medium flex items-center gap-2">
                    <Hash size={16}></Hash>
              ...
              <div className="flex items-center duration-300 absolute top-4 right-4">
                <div className="relative size-3 flex items-center justify-center">
                  {account.is_active && (
                    <div
                      className={`absolute size-3 top-0 left-0 rounded-full animate-ping bg-green-500`}
                    ></div>
                  )}
                  <div
                    className={`relative size-3 ${account.is_active ? 'bg-green-500' : 'bg-red-500'} rounded-full`}
                  ></div>
                </div>
              </div>
            </div>
            <div className="px-2 py-2 flex items-center gap-2 text-slate-800">
              <button
                onClick={() => handleOpenTab(account)}
                className={`${loading && 'opacity-50 cursor-not-allowed'} cursor-pointer flex-1 bg-slate-100 rounded-md text-sm hover:bg-slate-200 gap-2 px-2 py-2 flex items-center justify-center font-medium text-slate-600 duration-300`}
              >
                {loading && (
