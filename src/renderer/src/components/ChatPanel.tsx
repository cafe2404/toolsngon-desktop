...
const isEmptySupportMessage = (message: ChatMessage): boolean =>
  message.id.startsWith('empty_support_')
...
const getProductVideoUrl = (product?: UserProduct): string => {
  return (
    product?.product.guide_video_url ||
...
const getSupportTargetKey = (target: SupportTarget): string =>
...
const ChatPanel = ({ isVisible }: ChatPanelProps): React.JSX.Element => {
  ...
  const [selectedImage, setSelectedImage] = useState<{ file: File; previewUrl: string } | null>(
    null
  )
  ...
  const appendServerMessage = useCallback(
    (item: SupportChatMessageResponse, includeUser = false): void => {
  ...
  const appendServerNodeMessage = useCallback(
    (item: SupportChatMessageResponse, node: SupportNode, flow?: SupportFlowRuntime): void => {
      ...
      if (receivedSocketMessageIdsRef.current.has(item.id)) {
        setMessages((prev) =>
          prev.map((messageItem) =>
            messageItem.id === nextMessage.id ? { ...messageItem, ...nextMessage } : messageItem
          )
        )
        return
      }
  ...
  const hydrateCurrentConversation = useCallback(
    async (isCancelled: () => boolean = () => false): Promise<SupportHydrationResult> => {
      try {
        ...
        const matchedProduct = supportProducts.find(
          (item) =>
        ...
        const restoredTarget: SupportTarget = {
          ...
          appName:
            conversation.product_title || matchedProduct?.product.title || defaultTarget.appName,
          appLogo:
            conversation.product_logo_url ||
            matchedProduct?.product.logo_url ||
            defaultTarget.appLogo,
  ...
  const loadOlderMessages = useCallback(async (): Promise<void> => {
    if (!conversationId || !oldestServerMessageId || !hasOlderMessages || loadingOlderMessages)
      return
    const container = messageListRef.current
  ...
  useEffect(() => {
    ...
    const connect = async (): Promise<void> => {
      try {
        const { access } = await desktop.auth.tokens.get()
  ...
  const handleOpenGuide = (): void => {
    desktop.app.openExternal(getProductVideoUrl(supportTarget.product))
  }

  const handleOpenSupportGuide = (item: ChatMessage): void => {
    desktop.app.supportGuide.open({
  ...
  const handleCaptureScreenshot = async (): Promise<void> => {
    ...
    const image = await desktop.browser.tabs.captureScreenshot(currentTab.id)
    if (image) {
      clearSelectedImage()
      setScreenshot(image)
    }
  ...
  const getDesktopAction = (
    metadata?: Record<string, unknown>
  ): DesktopSupportAction | undefined => {
    const value =
      metadata?.desktop_action ||
      metadata?.desktopAction ||
      metadata?.app_action ||
      metadata?.appAction
  ...
  const runDesktopAction = async (action: ChatAction): Promise<void> => {
    ...
    if (desktopAction === 'navigate') {
      ...
      if (currentTab?.id && typeof url === 'string') {
        await desktop.browser.tabs.navigate(currentTab.id, url)
      }
      return
    }
    ...
    if (desktopAction === 'reload' || desktopAction === 'reload_current_tab') {
      await desktop.browser.tabs.reload(currentTab.id)
      return
    }
    ...
    if (desktopAction === 'inject_cookies' || desktopAction === 'reinject_cookies') {
      ...
      if (cookies && cookies.length > 0) {
        await desktop.browser.tabs.setCookies(currentTab.id, cookies)
        await desktop.browser.tabs.reload(currentTab.id)
      }
      return
    }
  ...
  const handleSupportButton = async (action: ChatAction, messageId: string): Promise<void> => {
    ...
    setMessages((prev) => [
      ...prev.map((item) => (item.id === messageId ? { ...item, actions: undefined } : item)),
      {
    ...
    try {
      ...
      if (res.data.message && responseNode) {
        ...
        setMessages((prev) => [
          ...prev,
          createSupportMessage(responseNode, res.data.flow || undefined)
        ])
      }
    } catch {
  ...
  const handleSubmit = async (event: FormEvent): Promise<void> => {
    ...
    const targetConversationId =
      conversationId || (await startSupportFlow(supportTargetRef.current))
    ...
    if (imageAttachment) {
      ...
      body.append(
        'attachment',
        dataUrlToFile(screenshotImage, `support-screenshot-${timestamp}.png`)
      )
    }
  ...
  return (
    <div
      className={`relative w-124 min-w-124 h-full bg-white border-l border-slate-200 overflow-hidden panel-slide-in flex flex-col ${isVisible ? '' : 'hidden'}`}
    >
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0 flex items-center gap-3">
            {supportTarget.appLogo && (
              <div className="size-10 min-w-10 overflow-hidden rounded-lg border border-slate-200 bg-white">
                <img
                  ...
                  src={supportTarget.appLogo}
                  alt={supportTarget.appName}
                />
              </div>
            )}
            <div className="min-w-0 flex flex-col justify-center gap-1">
              ...
              <span className="truncate text-xs leading-3 text-slate-800">
                {supportTarget.appName}
              </span>
            </div>
          </div>
      ...
      <div ref={messageListRef} className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        {messages.map((item, index) => {
          ...
          return item.role === 'status' ? (
            ...
            <div
              className={`flex w-full items-start gap-1.5 ${item.role === 'user' ? 'justify-end' : 'justify-start'}`}
              key={item.id}
            >
              {showSupportIdentity && (
              ...
              <div
                className={`flex flex-col gap-1 ${item.role === 'user' ? 'max-w-[80%] items-end' : 'max-w-[78%] items-start'}`}
              >
                {showSupportIdentity && (
                ...
                <div
                  className={`w-fit max-w-full rounded-lg px-3 py-2 text-sm ${
                    item.role === 'user' ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-700'
                  }`}
                >
                  <div>{item.text}</div>
                  {item.screenshot && (
                  ...
                  {item.guideTitle && (
                    <button
                      onClick={() => handleOpenSupportGuide(item)}
                      className="mt-2 text-left cursor-pointer min-w-62 rounded-xl border border-slate-200 bg-white p-3 text-slate-800 shadow-sm"
                    >
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="mt-1 text-[11px] font-medium uppercase tracking-wide text-slate-400">
                            {item.guideDescription ||
                              item.guideContentMarkdown?.slice(0, 24) + '...'}
                          </div>
                        </div>
                      </div>
                      {item.guideDescription && (
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {item.actions.map((action) => (
                      ...
                      >
                        ...
                        {action.type === 'support-flow' && action.action === 'open_url' && (
                          <VideoIcon size={13} />
                        )}
                        {action.label}
                      </button>
