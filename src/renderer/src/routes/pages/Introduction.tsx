    ...
    return (
        <div className={`w-full flex gap-12 flex-col p-24 h-full items-center justify-center overflow-y-auto`} style={{ display: currentTab?.id === "2" ? "flex" : "none" }}   >
            <div className="flex flex-col container mx-auto text-center">
                <h1 className="text-4xl whitespace-nowrap font-medium text-slate-800 mb-4">{t('introduction.title')}</h1>
                ...
                <div className="grid grid-cols-4 gap-6">
                    ...
                    <div className="relative w-full white rounded-lg border-white/50 h-full">
                        ...
                        <div className=" h-full flex w-full flex-col gap-4 text-slate-800 relative backdrop-blur-xs bg-white/10 p-5 text-center border border-slate-200 rounded-lg">
                            ...
                            <p className="text-slate-600">{t('introduction.issueDescriptionPrefix')} <strong>&lsquo;{t('introduction.issueDescriptionAction')}&lsquo;</strong> {t('introduction.issueDescriptionSuffix')}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <Dialog>
                        <form>
                            <DialogTrigger asChild>
                                <button className="flex items-center justify-center py-2 px-4 bg-slate-50 hover:bg-slate-100 rounded-md font-medium">{t('introduction.watchVideo')}</button>
                            </DialogTrigger>
                            <DialogContent className="max-w-[70vw] !w-[70vw]">
                                <DialogHeader>
                                    <DialogTitle>{t('introduction.videoTitle')}</DialogTitle>
                                    <DialogDescription>
                                        {t('introduction.videoDescription')}
                                    </DialogDescription>
                                </DialogHeader>
